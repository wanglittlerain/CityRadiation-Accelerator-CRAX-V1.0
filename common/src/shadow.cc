...
void Shadow::forward(Surface& srf, Polyhedron& phd, const ep3& sv, int type, bool save) const {
    ...
    auto fun = [&](const epts& ps, double fh) {
        ...
        if (area > sarea || area > srf._area + c_1e_2) return false;
        ++num;
...
bool Shadow::_projection(bg_polygon& poly, const Surface& srf, const ep3& sv, 
    ...
    auto insert = [&](const ep3& p0, const ep3& p1) {
        ...
        const auto ddot{srf._normi.dot(dir)};
        const auto t{(srf._d - p0.dot(srf._normi)) / ddot};
    ...
    auto proj = [&](ep3 pt) {
        pt(2) = terrain(pt(2));
        auto before{false};
        const auto t{(srf._d - pt.dot(srf._normi)) / ndot};
        if (t > c_1e_8) {
            before = true;
            sp = pt + t * sv;
        }
...
bool Shadow::projection_(const Surface* nsrf, const ep3& sv, const ep3& p) const {
    ...
    auto t{(nsrf->_d - nsrf->_normi.dot(p)) / nsrf->_normi.dot(sv)};
    ep3 ap{t * sv};
